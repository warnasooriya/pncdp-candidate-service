...
exports.updateUserRole = async (req, res) => {
    try {
        const { id, userTypes } = req.body;

        if (!id || !userTypes) {
            return res.status(400).json({ error: 'id and userTypes are required' });
        }

        const allowed = ['Candidate', 'Recruiter'];
        if (!allowed.includes(userTypes)) {
            return res.status(400).json({ error: 'Invalid userTypes' });
        }
